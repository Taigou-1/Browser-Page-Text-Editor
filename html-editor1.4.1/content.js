...
async function saveChanges() {
  ...
  modifiedElements.forEach(({original, modified, xpath}, element) => {
    ...
    if (targetNode) {
      // 查找原始文本位置
      const originalText = targetNode.innerHTML;
      const start = modifiedContent.indexOf(originalText);
      
      if (start !== -1) {
        modifiedContent = 
          modifiedContent.slice(0, start) +
          modified +
          modifiedContent.slice(start + originalText.length);
      }
    }
  });
